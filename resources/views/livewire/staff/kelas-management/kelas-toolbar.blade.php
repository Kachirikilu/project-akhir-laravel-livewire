    ...
    <div class="ml-auto">
        <flux:dropdown>
            ...
            <flux:menu class="min-w-48 !bg-[var(--second-pop-up-color)] !border-[var(--border-table-color)] !text-[var(--contrast-main-text)]">
                ...
                <flux:menu.item
                    @click="
                        $store.kelas?.setType(1);
                        $store.kelas?.setEdit(0);
                        {{-- $store.kelas?.resetSelect(); --}}
                ...
                <flux:menu.item
                    @click="
                        $store.kelas?.setType(2);
                        $store.kelas?.setEdit(0);
                        {{-- $store.kelas?.resetSelect(); --}}
                        $store.kelas?.setColor('text-amber-700 dark:text-amber-400');
                        $flux.modal('kelas-modal').show();
                        $wire.addMK(2);
                    "
                ...
                <flux:menu.item
                    @click="
                        $store.kelas?.setType(3);
                        $store.kelas?.setEdit(0);
                        {{-- $store.kelas?.resetSelect(); --}}
                        $store.kelas?.setColor('text-indigo-700 dark:text-indigo-400');
                        $flux.modal('kelas-modal').show();
                        $wire.addMK(3);
                    "
                ...
                <flux:menu.item
                    @click="
                        $store.kelas?.setType(4);
                        $store.kelas?.setEdit(0);
                        {{-- $store.kelas?.resetSelect(); --}}
                        $store.kelas?.setColor('text-red-700 dark:text-red-400');
                        $flux.modal('kelas-modal').show();
                        $wire.addMK(4);
                    "
