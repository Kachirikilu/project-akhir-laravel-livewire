    ...
    <div class="space-y-4">
        ...
        <template x-if="$store.{{ $alpine ?? 'config' }}?.isEdit == 1">
            @include('livewire.global.modal-form.loading-animation', [
                'wireLoading' =>
                    'add' . ($wireLoading ?? $nameXString) . ', edit' . ($wireLoading ?? $nameXString),
            ])
        </template>
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-zinc-500 uppercase tracking-widest mt-1">
                    Daftar MK yang menggunakan {{ $nameXString }} ini
                </p>
            </div>
        </div>
                        ...
                        <div wire:key="rps-row-{{ $r['id'] }}-{{ $nameXString }}" x-data="{ expanded: false }"
                            ...
                            <div x-on:click="expanded = !expanded"
                                ...
                                <div class="flex items-center gap-2 flex-shrink-0">
                                    @if ($r['draf'])
                                        <flux:badge size="xs" color="yellow">{{ $r['draf_text'] }}</flux:badge>
                                    @else
                                        <flux:badge size="xs" color="green">{{ $r['draf_text'] }}</flux:badge>
                                    @endif
                                    ...
                                    <div
                                        ...
                                        <div>
                                            <span
                                                ...
                                                RPS:</span>
                                            <p class="text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed mt-1">
                                                {{ $r['deskripsi'] ?? '-' }}
                                            </p>
                                        </div>
                                        ...
                                        <div
                                            class="flex items-center gap-4 pt-2 border-t border-zinc-200/50 dark:border-zinc-700/50">
                                            <button type="button"
                                                class="cursor-pointer group flex items-center gap-1.5 text-xs font-medium text-zinc-500 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors"
                                                @click="..." wire:click="editRPS('{{ $r['id'] }}')">
                                                <div
