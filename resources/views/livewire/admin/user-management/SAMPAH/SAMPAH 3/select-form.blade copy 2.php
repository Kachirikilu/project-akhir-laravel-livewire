@php $safeModel = str_replace('.', '_', $modelString); @endphp

<div class="relative" 
     wire:key="container-select-{{ $safeModel }}-{{ time() }}"
     x-data="{
        open_{{ $safeModel }}: false,
        value_{{ $safeModel }}: @entangle($modelString)
     }">
    ...
    <div class="relative mt-2">
        ...
        <input autocomplete="off" x-model="value_{{ $safeModel }}" {{-- Gunakan x-model agar Alpine tahu isinya --}} type="text" readonly @click="open_{{ $safeModel }} = true"
            @click.outside="open_{{ $safeModel }} = false" @keydown.escape.window="open_{{ $safeModel }} = false" id="{{ $modelString }}"
        ...
        <button x-show="typeof value_{{ $safeModel }} !== 'undefined' && value_{{ $safeModel }}" x-transition:enter="transition ease-out duration-200"
            x-transition:enter-start="opacity-0 scale-50" x-transition:enter-end="opacity-100 scale-100"
            x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100"
            x-transition:leave-end="opacity-0 scale-50" type="button" @click="if(typeof value_{{ $safeModel }} !== 'undefined') value_{{ $safeModel }} = ''"
    ...
    {{-- Dropdown Result --}}
    <div x-show="open_{{ $safeModel }}" x-transition.opacity x-cloak
        ...
        @foreach ($xOptions as $option)
            <div wire:key="option-{{ $option }}" @click="value_{{ $safeModel }} = '{{ $option }}'; open_{{ $safeModel }} = false"
