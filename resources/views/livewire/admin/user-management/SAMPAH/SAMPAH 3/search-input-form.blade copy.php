<div class="relative" x-data="{
    open{{ $typeXString }}: false,
    search{{ $typeXString }}: @entangle($nameSearchString),
    selectedId{{ $typeXString }}: @entangle($idString).live
}">
    ...
    <div class="relative mt-2">
        ...
        <input x-model="search{{ $typeXString }}" autocomplete="off" type="text"
            @focus="
        open{{ $typeXString }} = true;
        $event.target.select();
        $wire.{{ $fetchString }}(search{{ $typeXString }});
    "
            @input.debounce.300ms="
        open{{ $typeXString }} = true;
        $wire.{{ $fetchString }}(search{{ $typeXString }});
    "
            @click.outside="open{{ $typeXString }} = false" @keydown.escape.window="open{{ $typeXString }} = false"
        ...
        @include('livewire.admin.global.search-and-filters.partial.reset-button', [
            'xShow' => "search$typeXString || selectedId$typeXString",
            'xSearch' => "search$typeXString",
            'xClick' => "search$typeXString = ''; selectedId$typeXString = null; open$typeXString = false",
            'xWire' => $resetXInput,
            'xWire2' => $fetchString . '()',
            'xColor' => $colorIcon,
        ])
    ...
    {{-- Info Terpilih --}}
    <div x-show="selectedId{{ $typeXString }} && search{{ $typeXString }}" x-cloak>
        <p class="text-xs text-indigo-600 mt-1 font-medium italic">
            Terpilih: <span x-text="search{{ $typeXString }}"></span> (ID: <span
                x-text="selectedId{{ $typeXString }}"></span>)
        </p>
    </div>

    {{-- DROPDOWN HASIL --}}
    <div x-show="open{{ $typeXString }}" x-transition.opacity x-cloak
        ...
        @forelse ($xResults as $x)
            <div wire:key="{{ $x[$typeXString] }}-{{ $x['id'] }}"
                @click="
                    search{{ $typeXString }} = '{{ (isset($noName) ? '' : $nameXString . ' ') . $x[$typeXString] }}'; 
                    selectedId{{ $typeXString }} = {{ $x['id'] }}; 
                    open{{ $typeXString }} = false; 
