<div class="relative" x-data="{
    ...
    search: @entangle('jurusan_name_search'),
    ...
    <div class="relative mt-2">
        ...
        <input x-model="search" autocomplete="off" type="text" {{-- SAAT FOKUS: Buka dropdown & muat data default --}}
            ...
            @input.debounce.300ms="
                open = true;
                $wire.fetchJurusan(search); 
            "
            @click.outside="open = false" @keydown.escape.window="open = false" id="jurusan_search"
        ...
        <button x-show="search || selectedId" type="button" x-transition:enter="transition ease-out duration-200"
            ...
            @click="search = ''; selectedId = null; $wire.resetJurusanInput(); $wire.fetchJurusan()"
    ...
    <div x-show="open" x-transition.opacity x-cloak
        ...
        @forelse ($jurusan_results as $jurusan)
            <div wire:key="jurusan-{{ $jurusan['id'] }}"
                @click="
                    search = 'Jurusan {{ $jurusan['jurusan'] }}'; 
                    selectedId = {{ $jurusan['id'] }}; 
                    open = false; 
                    $wire.selectJurusan({{ $jurusan['id'] }}, '{{ $jurusan['jurusan'] }}')
                "
                ...
                <div class="flex justify-between items-center">
                    <div class="flex flex-col">
                        <span class="font-semibold text-gray-800 leading-tight">Jurusan {{ $jurusan['jurusan'] }}</span>
                        <span class="text-xs text-gray-500 mt-0.5">Fakultas {{ $jurusan['fakultas'] }}</span>
                    </div>
