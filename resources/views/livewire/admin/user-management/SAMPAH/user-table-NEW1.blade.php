<x-admin.global.table.main-layout-table>
    ...
    <x-slot:header>
        {{-- ID - Sorting Angka --}}
        @include('livewire.global.table.head-table', ['sortFieldString' => 'id', 'isMain' => 1, 'isCenter' => 1])
        ...
        {{-- Name - Sorting A-Z --}}
        @include('livewire.global.table.head-table', ['sortFieldString' => 'name', 'isMain' => 1])
        ...
        {{-- Prodi - Sorting A-Z --}}
        @include('livewire.global.table.head-table', ['sortFieldString' => 'prodi', 'isMain' => 1])
        @include('livewire.global.table.head-table', ['sortFieldString' => 'status', 'isCenter' => 1])
    ...
    @forelse($users as $user)
        ...
        <tr wire:key="user-{{ $user->id }}" class="hover:bg-gray-50" data-user-id="{{ $user->id }}">
            ...
            @include('livewire.global.table.body-table', [
                'xData' =>  $user->id ?? '-',
                'sortFieldString' => 'id',
                'isMain' => 1,
                'isCenter' => 1
            ...
            @include('livewire.global.table.body-table', ['xData' =>  $user->identity1 ?? '-', 'sortFieldString' => 'identity1'])
            @if ($filter != 'mahasiswa')
                @include('livewire.global.table.body-table', ['xData' =>  $user->identity2 ?? '-', 'sortFieldString' => 'identity2'])
            @endif
            @if ($filter == 'dosen' || $filter == '')
                @include('livewire.global.table.body-table', ['xData' =>  $user->identity3 ?? '-', 'sortFieldString' => 'identity3'])
            @endif

            @include('livewire.global.table.body-table', ['xData' =>  $user->email ?? '-', 'sortFieldString' => 'email'])

            @if ($filter == 'mahasiswa')
                @include('livewire.global.table.body-table', ['xData' => $detail->tahun_angkatan ?? '-', 'sortFieldString' => 'tahun_angkatan'])
            @endif
