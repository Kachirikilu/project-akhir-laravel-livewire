            ...
            @if (!empty($isKode) && $isKode > 0) maxlength="{{ $isKode }}"
                ...
                oninput="
                    this.value = this.value
                        ...
                        .slice(0, {{ $isKode }})
                "
            @elseif (isset($numberOnly) && $numberOnly) inputmode="numeric"
                {{ ($noZero ?? null) ? 'pattern="[1-9]*"' : 'pattern="[0-9]*"' }} 
                 maxlength="{{ $maxlength ?? 255 }}"
                oninput="this.value = this.value.replace(/{{ ($noZero ?? null) ? '[^1-9]' : '[^0-9]' }}/g, '').slice(0, {{ $maxlength ?? 255 }})"
            @else
