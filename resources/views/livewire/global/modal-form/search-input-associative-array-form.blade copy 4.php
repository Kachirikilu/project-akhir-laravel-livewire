<div class="relative" wire:key="search-array-{{ $selectX }}" x-data="{
    ...
    init() {
        ...
        if (this.subItems.length === 0) {
            this.subItems = this.items.map(() => ({ scpmk: [], cpl: [], ref: [] }));
        }
    ...
    addItem(id, name, kode, subData, cplData, refData) {
        ...
        if (!this.items.map(i => Number(i)).includes(normalizedId)) {
            ...
            this.subItems.push({
                scpmk: subData || [],
                cpl: cplData || [], // Simpan CPL
                ref: refData || [] // Simpan Referensi
            });
        }
        this.search = '';
    },
    ...
    get totalSubCPMK() {
        return this.subItems.reduce((total, item) => {
            // Akses ke properti .scpmk
            return total + (Array.isArray(item.scpmk) ? item.scpmk.length : 0);
        }, 0);
    }
}">
                        ...
                        <div class="flex items-center gap-3 cursor-pointer flex-1"
                            ...
                            <div class="flex flex-col">
                                ...
                                <div class="text-xs mt-1">
                                    ...
                                    <span>
                                        Bobot: <span
                                            x-text="Array.isArray(subItems[index]) 
    ? subItems[index].reduce((t, s) => t + Number(s.bobot || 0), 0) 
    : 0">
                                        </span>%
                                    </span>
                                </div>
                    ...
                    {{-- Di dalam template x-for="(id, index) in items" --}}
<div x-show="expanded.includes(index)" x-collapse>
    <div class="px-4 bg-white/20 dark:bg-black/5">
        <div class="border-t border-[var(--border-table-color)] pt-3 overflow-x-auto scrollbar-thin">
            <table class="w-full text-[11px] text-left border-collapse min-w-[500px]">
                <thead>
                    <tr class="text-gray-400 uppercase tracking-tighter border-b border-[var(--border-table-color)]">
                        ...
                        <th class="pb-3 px-4 min-w-24">Tugas</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-[var(--border-table-color)]">
                    {{-- FIX: Loop ke properti .scpmk --}}
                    <template x-for="sub in (subItems[index]?.scpmk || [])" :key="sub.id">
                        <tr>
                            <td class="py-2.5 px-2 font-bold">
                                <flux:badge color="fuchsia" size="sm" class="text-[9px]">
                                    <span x-text="sub.kode || '-'"></span>
                                </flux:badge>
                            </td>
                            <td class="py-2.5 px-2" x-text="sub.deskripsi || '-'"></td>
                            <td class="py-2.5 px-2" x-text="sub.materi || '-'"></td>
                            <td class="py-2.5 px-2" x-text="sub.metodologi || '-'"></td>
                            <td class="py-2.5 px-2" x-text="sub.indikator || '-'"></td>
                            <td class="py-2.5 px-2 text-center">
                                <span class="px-2 py-0.5 bg-gray-100 rounded text-[9px]" x-text="sub.metode"></span>
                            </td>
                            <td class="py-2.5 px-2 text-center font-bold" x-text="sub.bobot + '%'"></td>
                            <td class="py-2.5 px-2" x-text="sub.tugas || '-'"></td>
                        </tr>
                    </template>
                </tbody>
                <tfoot>
                    <tr class="border-t-2 border-double border-[var(--border-table-color)]">
                        <td colspan="6" class="py-4 font-bold text-xs uppercase">Total Bobot CPMK Ini:</td>
                        <td class="py-4 text-center font-black text-sm" 
                            x-text="(subItems[index]?.scpmk?.reduce((t, s) => t + Number(s.bobot || 0), 0) || 0) + '%'">
                        </td>
                    </tr>
                </tfoot>
            </table>

            {{-- Bagian CPL & Referensi --}}
            <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 pb-4">
                <div class="bg-white/30 p-3 rounded-lg border border-[var(--border-table-color)]">
                    <h4 class="text-[10px] font-bold uppercase text-gray-500 mb-2">Dukungan CPL</h4>
                    <div class="flex flex-wrap gap-1">
                        {{-- FIX: Akses subItems[index].cpl --}}
                        <template x-for="cpl in (subItems[index]?.cpl || [])" :key="cpl.id">
                            <flux:badge color="zinc" size="sm" x-text="cpl.kode"></flux:badge>
                        </template>
                    </div>
                </div>

                <div class="bg-white/30 p-3 rounded-lg border border-[var(--border-table-color)]">
                    <h4 class="text-[10px] font-bold uppercase text-gray-500 mb-2">Referensi Utama</h4>
                    <ul class="space-y-1">
                        {{-- FIX: Akses subItems[index].ref --}}
                        <template x-for="ref in (subItems[index]?.ref || [])" :key="ref.id">
                            <li class="text-[10px] leading-tight border-l-2 border-blue-500 pl-2">
                                <span class="font-bold" x-text="ref.penulis + ' (' + (ref.tahun || '-') + ')'"></span><br>
                                <span class="italic text-gray-500" x-text="ref.judul"></span>
                            </li>
                        </template>
                    </ul>
                </div>
