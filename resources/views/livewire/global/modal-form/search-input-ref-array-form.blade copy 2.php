<div class="relative" wire:key="search-array-{{ $selectX }}" x-data="{
    ...
    if (this.subItems.length === 0 && this.items.length > 0) {
        // Jika sedang mode edit, petakan data yang sudah ada (pastikan dikirim dari backend)
        this.subItems = this.items.map((id, index) => ({
            ref: this.initialRefs ? this.initialRefs[index] : [], 
            // initialRefs harus di-entangle dari backend jika ingin persistence
        }));
    }
}
