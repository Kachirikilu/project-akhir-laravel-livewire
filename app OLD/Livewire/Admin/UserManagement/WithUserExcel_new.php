...
trait WithUserExcel
{
    ...
    public function importExcel()
    {
        ...
        foreach ($dataRows as $excelIndex => $row) {
            ...
            // Lookup prodi_id untuk setiap baris menggunakan program studi dari Excel
            $prodiId = null;
            $prodiName = $data['program studi'] ?? null;
            if ($prodiName) {
                $prodiId = Prodi::where('nama_prodi', $prodiName)->value('id');
            }
            ...
            $messages = collect($validator->errors()->messages())
                ->map(function($errors, $field) {
                    if ($field === 'prodi_id') {
                        ...
                        if (strpos($errorStr, 'required') !== false) {
                            return 'Program studi wajib dipilih.';
                        } elseif (strpos($errorStr, 'exists') !== false) {
                            return 'Program studi yang dipilih tidak valid.';
                        }
                    }
    ...
    private function mapExcelToProperties($data)
    {
        ...
        // Gunakan prodi_id dari parsed row data (sudah di-lookup saat import)
        $this->prodi_id = $data['prodi_id'] ?? null;
