...
trait WithProdiSearchFilters
{
    ...
    public function inputProdiFilter()
    {
        ...
        if ((strlen($search) > 1 || is_numeric($search)) && ! $this->prodi_name) {
            $this->prodiSearchResults = Prodi::query()
    ...
    //     $jurusanId = $userProdi->jurusan_id;
    //     $fakultasId = $userProdi->jurusan_rel?->fakultas_id;
    ...
    //     } else {
    //         $query->where('jurusans.fakultas_id', $fakultasId);
    //     }
    ...
    //             ELSE 3
    //         END ASC
    //     ', [$namaProdiUser, $jurusanId, $fakultasId])
    ...
    //             ->join('jurusans', 'prodis.jurusan_id', '=', 'jurusans.id')
    //             ->join('fakultas', 'jurusans.fakultas_id', '=', 'fakultas.id')
    //             ->where('jurusans.fakultas_id', '!=', $fakultasId)
