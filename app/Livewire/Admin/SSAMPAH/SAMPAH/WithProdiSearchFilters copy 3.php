...
trait WithProdiSearchFilters
{
    ...
    //             $this->prodi_kode = $exactMatch->kode_pr
    ...
    public function updatedProdiNameSearch($value)
{
    ...
    if (strlen($value) > 0) {
        ...
        if ($exactMatch) {
            $this->prodi_id = $exactMatch->id;
            $this->prodi_kode = $exactMatch->kode_pr ?: ($exactMatch->jurusan_rel?->kode_jr ?: 'UNI');
            $this->prodiNameSearch = $exactMatch->nama_prodi;
            $this->prodiResults = [];
        }
    } 
    ...
    public function selectProdi($id, $prodiName)
    {
        ...
        $data = Prodi::with(['jurusan_rel.fakultas_rel'])->find($id);
        if ($data) {
            $this->prodi_kode = $data->kode_pr ?? $data->jurusan_rel->kode_jr ?? $data->jurusan_rel->fakultas_rel->kode_fk ?? 'UNI';
        }
    ...
    public function selectProdiArray($id)
    {
        ...
        if ($data && !in_array($id, $this->prodi_id_array)) {
            ...
            $this->prodi_name_array[] = $data->nama_prodi;
            $this->prodi_kode_array[] = $data->kode_pr ?? 'UNI';
        }
    }
