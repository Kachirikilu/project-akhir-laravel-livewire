...
class MataKuliah extends Model
{
    ...
    protected $appends = ['tingkata_mode', 'kode', 'kode_blok', 'matkul', 'sks_tm', 'sks_pr', 'sks_pl', 'sks_sm', 'tipeSksText'];
    ...
    protected function kode(): Attribute
    {
        return Attribute::get(function () {
            ...
            $prefixDefault = strtoupper($this->kode_mk);
            $prodi = $this->prodis->first();

            if ($prodi) {
                if ($this->tingkatan_mk == 1) { // Tingkat Prodi
                    $prefix = $prodi->kode ?? $prodi->jurusan_rel->fakultas_rel->kode_fk ?? $prefixDefault ?? 'UNI'; // Memanggil accessor 'kode' di Model Prodi Anda
