...
class KelasJadwal extends Model
{
    ...
    return $query->where(function ($q) use ($search, $searchTerm, $searchLower, $searchClean) {
        $q->where(function ($jq) use ($searchClean) {
            ...
            if ($label) $jq->where('label_kelas', 'LIKE', "%{$label}%");
            if ($wilayah) $jq->where('kode_wilayah', 'LIKE', "%{$wilayah}%");
            if ($tahun) {
                if (strlen($tahun) >= 4) $tahun = substr($tahun, -2);
        ...
        if (is_numeric($search)) $q->orWhere('kelas_jadwals.id', $search);
    });
    }
}
